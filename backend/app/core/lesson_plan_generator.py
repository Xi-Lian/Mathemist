...
class LessonPlanGenerator:
    ...
    def _load_lesson_plan_common_characteristics(self) -> str:
        ...
        try:
            ...
            file_path = base_dir / "learning_resource" / "教案" / "优秀教案共性整合（最终版）.md"
            print(f"📂 尝试加载优秀教案共性文件: {file_path}")
            if file_path.exists():
    ...
    def _load_theory_cards(self) -> str:
        ...
        try:
            base_dir = Path(__file__).parent.parent.parent.parent
            file_path = base_dir / "learning_resource" / "理论卡片" / "理论卡片.md"
            print(f"📂 尝试加载理论卡片文件: {file_path}")
            if file_path.exists():
                with open(file_path, 'r', encoding='utf-8') as f:
                    content = f.read()
...
### 一、整体结构要求
请按照以下结构组织教案，每个部分都要清晰标注理论依据（必须来自理论卡片）：
...
### 1. 知识与技能目标
[具体描述学生应该掌握的知识点和技能]
**📌 理论依据：[理论卡片X：理论名称] - [理论核心观点] - 应用场景：[说明该理论如何指导本环节设计]**
...
| 理论依据 | 应用环节 | 理论核心观点 | 具体作用 |
|---------|---------|-------------|---------|
| [理论卡片X：理论名称] | [环节名称] | [理论核心观点] | [说明该理论在该环节的具体作用] |
| [理论卡片X：理论名称] | [环节名称] | [理论核心观点] | [说明该理论在该环节的具体作用] |
| ... | ... | ... | ... |
...
#### 1. 引用格式
在每个教学环节的设计说明后，必须使用以下格式标注理论依据（理论依据仅限于理论卡片资源）：
