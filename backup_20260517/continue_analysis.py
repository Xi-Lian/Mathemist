...
def process_exercise(exercise_id, resource, analyzer, builder, output_dir, progress_queue):
    try:
        ...
        original_resource = resource.get('original_resource', {}) if 'original_resource' in resource else resource
...
def continue_analysis():
    print("=" * 60)
    print("Continuing exercise analysis from where it left off")
    print("=" * 60)
    ...
    print(f"Already analyzed unique questions: {len(existing_signatures)}")
    ...
    for resource_type, resources in exercise_resources.items():
        for i, resource in enumerate(resources):
            ...
            content_signature = f"{title}||{question}"
            
            if content_signature in existing_signatures:
                continue
            
            if content_signature in added_signatures:
                continue
            
            exercise_id = f"{resource_type}_{i}_{hash(source_file + title) % 100000}"
            
            added_signatures.add(content_signature)
            all_tasks.append((exercise_id, resource))
    
    print(f"Need to analyze (after deduplication): {len(all_tasks)} exercises")
