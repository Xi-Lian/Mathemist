"""
检查余弦定理相关的课件类型分布
"""

import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), 'backend'))

from app.core.vector_database_builder import VectorDatabaseBuilder

vdb = VectorDatabaseBuilder('learning_resource')
client = vdb.get_chroma_client()
coll = client.get_collection('math_resources_geometry')

# 获取所有课件资源
results = coll.get(
    where={'resource_type': 'courseware'},
    include=['metadatas']
)

print("=" * 80)
print("查找余弦定理相关的课件")
print("=" * 80)

cosine_theorem_all = []
for metadata in results['metadatas']:
    filename = metadata.get('文件名', '')
    title = metadata.get('标题', '')
    teaching_use = metadata.get('教学用途', '')
    
    if '余弦定理' in filename or '余弦定理' in title:
        cosine_theorem_all.append({
            '文件名': filename,
            '标题': title[:80],
            '教学用途': teaching_use,
        })

print(f"\n总共找到 {len(cosine_theorem_all)} 个余弦定理相关课件，教学用途分布:")
use_count = {}
for cw in cosine_theorem_all:
    use = cw['教学用途']
    use_count[use] = use_count.get(use, 0) + 1

for use, count in sorted(use_count.items(), key=lambda x: -x[1]):
    print(f"  {use}: {count}个")

print("\n详细列表:")
for i, cw in enumerate(cosine_theorem_all[:10], 1):
    print(f"{i}. {cw['文件名']}")
    print(f"   教学用途: {cw['教学用途']}")
    print()

if len(cosine_theorem_all) == 0:
    print("未找到任何余弦定理相关的课件")
